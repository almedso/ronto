    ...
    def __init__(cls, model):
        ...
        if 'repo' in model:
            ...
            if isinstance(model['repo'], dict):
                if 'url' in model['repo'] \
                and isinstance(model['repo']['url'], str):
                    cls.url = model['repo']['url']
                if 'manifest' in model['repo'] \
                and isinstance(model['repo']['manifest'], str):
                    cls.url = model['repo']['manifest']
                if 'branch' in model['repo'] \
                and isinstance(model['repo']['branch'], str):
                    cls.url = model['repo']['branch']

    def fetch(cls):
        if cls.url != '':
            verbose(f"Init repo from {cls.url}")
            run_cmd(['repo', 'init', '-u', cls.url,
                    '-m', cls.manifest, '-b', cls.branch])
            verbose(f"Sync repo")
            run_cmd(['repo', 'sync'])

...
def get_init_build_dir(model):
    ...
    if 'build' in model and isinstance(model['build'], dict):
        if 'build_dir' in model['build'] \
        and isinstance(model['build']['build_dir'], str):
...
def get_init_template_dir(model):
    template_dir = None
    if 'build' in model and isinstance(model['build'], dict):
        if 'template_dir' in model['build'] \
        and isinstance(model['build']['template_dir'], str):
...
def get_init_script(model):
    ...
    script = INIT_SCRIPT
    if 'build' in model and isinstance(model['build'], dict):
        if 'init_script' in model['build'] \
        and isinstance(model['build']['init_script'], str):
...
class SiteConfigHandler:
    ...
    def __init__(self, model):
        ...
        if 'build' in model and isinstance(model['build'], dict):
            if 'site' in model['build'] \
            and isinstance(model['build']['site'], dict):
                if 'overwrite' in model['build']['site']:
                    self.overwrite = model['build']['site']['overwrite']
                    verbose(f"Overwrite site.conf: {self.overwrite}")
                if 'file' in model['build']['site'] \
                and isinstance(model['build']['site']['file'], str):
